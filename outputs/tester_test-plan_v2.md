# Sir Reginald Test Plan v2

**Application:** Sir Reginald Makesworth III - Your Distinguished Workshop Guardian
**Version:** 2.0
**Date:** January 16, 2026
**Tester:** Tester Agent
**Status:** Updated with Critic Feedback
**Changes from v1:** Added 18 new tests addressing critic's 10 identified gaps
...
| F36 | Sensitivity Slider - Paranoid Impact **(NEW)** | 1. Set sensitivity to "Paranoid" 2. Show hand near tool at safe distance 3. Compare to "Relaxed" level | At "Paranoid", Sir Reginald warns about minor issues (e.g., hand at moderate distance) that would NOT trigger at "Relaxed" level | P1 |
...
| G22 | Personality Arc Progression **(NEW)** | 1. Use app for 15+ minutes continuously 2. Trigger multiple responses throughout 3. Compare early vs late responses | After 15+ minutes, Sir Reginald uses warmer language: "Between you and me...", refers to self as "Reggie", less formal phrasing than initial responses | P1 |
| G23 | Name Usage Frequency **(NEW)** | 1. Complete onboarding with name 2. Trigger 10 separate responses 3. Count how many include user's name | Name appears in responses every 3-5 interactions (not every sentence, not never) - approximately 2-3 times out of 10 responses | P1 |
| G24 | Token Endpoint Personalization **(NEW)** | 1. Complete onboarding with name "James" 2. Check Network tab for /api/token request 3. Inspect response payload | /api/token response includes userName in system prompt or configuration data that personalizes the session | P0 |
| G25 | Visual Overlay Keyword Mapping **(NEW)** | 1. Trigger response containing "your hand" or "fingers" or "grip" 2. Trigger response containing "safety spectacles" or "eye protection" 3. Trigger response containing "that blade" or "the saw" | Keywords map to correct overlay regions: hand keywords -> hands region highlighted, eye keywords -> face region highlighted, tool keywords -> tool region highlighted | P1 |
| G26 | Session Resumption After Token Refresh **(NEW)** | 1. Use app until token refresh occurs (~25 min) 2. Verify token refresh happens 3. Continue using app | After token refresh, session continues without requiring re-setup; resumptionToken or similar mechanism allows seamless continuation | P1 |
...
| D5a | Hearing Protection - Multi-Modal Detection **(PROMOTED TO P1)** | 1. Play saw sound (60dB+) through speakers 2. Show ears visible in frame 3. Repeat 10 times 4. Record success rate | Sir Reginald detects BOTH audio (saw sound) AND visual (exposed ears) and says something like "I can hear that saw AND see your hand" - Target: >70% success rate (7/10 trials) | P1 |
...
| E17a | Backup Audio - Connection Lost **(NEW)** | 1. Simulate connection loss 2. Listen for audio fallback | connection-lost.mp3 plays with Sir Reginald saying something like "I've lost my connection to the workshop..." | P1 |
| E17b | Backup Audio - Reconnecting **(NEW)** | 1. During reconnection attempt 2. Listen for audio | reconnecting.mp3 plays with Sir Reginald saying "Just a moment, re-establishing my vigil..." or similar | P1 |
| E17c | Backup Audio - Safety Reminder **(NEW)** | 1. Enter degraded/offline mode 2. Listen for periodic audio | safety-reminder.mp3 plays periodically in degraded mode: "Do mind your safety whilst I'm momentarily indisposed..." | P1 |
| E17d | Backup Audio - Connection Restored **(NEW)** | 1. Successfully reconnect after disconnect 2. Listen for audio | connection-restored.mp3 plays: "Ah, splendid! Back in action..." or similar celebratory phrase | P1 |
| E18 | Browser Refresh | 1. Refresh page mid-session | App restarts cleanly (onboarding or resume) | P1 |
| E19 | Offline Cached Phrase Mapping **(NEW)** | 1. Identify all failure types: connection-lost, reconnecting, degraded-mode, connection-restored 2. Trigger each failure type 3. Verify correct audio plays | Each specific failure type triggers its corresponding cached MP3: connection-lost -> connection-lost.mp3, reconnecting -> reconnecting.mp3, degraded -> safety-reminder.mp3, restored -> connection-restored.mp3 | P1 |
...
| DF3a | Camera Setup - Greeting Stage **(NEW)** | 1. Enter camera setup from onboarding 2. Observe first stage | Sir Reginald's welcome dialogue appears: "Ah, there you are [name]!" or similar greeting, explaining what camera setup involves | P0 |
| DF3b | Camera Setup - Positioning Stage **(NEW)** | 1. Progress past greeting 2. Observe positioning stage | Guide overlay appears with dashed border, label "Position your workspace within this frame", Sir Reginald provides theatrical direction like "A bit to the left... perfect!" | P0 |
| DF3c | Camera Setup - Survey Stage **(NEW)** | 1. Confirm positioning 2. Observe survey stage | Pulsing animation overlay while Sir Reginald scans workspace, spinning indicator with "Surveying..." text, Sir Reginald comments on what he sees | P0 |
| DF3d | Camera Setup - Complete Stage **(NEW)** | 1. Wait for survey to complete 2. Observe final stage | Survey animation stops, "Begin Monitoring" button appears prominently, Sir Reginald confirms readiness: "I'm prepared to begin my vigil over [name]'s workshop" | P0 |
...
| Functional Tests | 36 tests (+1 new) | 65 minutes |
| Gemini Integration Tests | 26 tests (+5 new) | 60 minutes |
| Demo Scenario Tests | 15 tests (+1 promoted) | 130 minutes (includes repeat trials) |
| Error Handling Tests | 24 tests (+6 new) | 60 minutes |
| Demo Flow Tests | 26 tests (+4 new) | 100 minutes |
| Accessibility & UX Tests | 20 tests | 30 minutes |
| **Total** | **202 tests** | **~9 hours** |

### New Tests Added (v2)

| Gap # | Gap Description | New Test IDs | Count |
|-------|-----------------|--------------|-------|
| 1 | Camera Setup 4 Stages Not Verified | DF3a, DF3b, DF3c, DF3d | 4 |
| 2 | Personality Arc Progression Untested | G22 | 1 |
| 3 | Name Frequency Not Verified | G23 | 1 |
| 4 | Backup Audio Files Playback Not Tested | E17a, E17b, E17c, E17d | 4 |
| 5 | Multi-Modal Detection Needs More Trials | D5a (promoted to P1 + 10 trials) | 1 |
| 6 | Token Endpoint Personalization Not Tested | G24 | 1 |
| 7 | Visual Overlay Keyword Triggers Not Tested | G25 | 1 |
| 8 | Session Resumption After Token Refresh | G26 | 1 |
| 9 | Sensitivity Slider Impact Unverified | F36 | 1 |
| 10 | Offline Cached Phrase Mapping Untested | E19 | 1 |
| **Total** | | | **18** |

### Recommended Testing Order

1. **Day 1 (3 hours):** Visual/UI Tests, Functional Tests (including new F36)
2. **Day 2 (3.5 hours):** Gemini Integration Tests (including new G22-G26), Error Handling Tests (including new E17a-E19)
3. **Day 3 (4.5 hours):** Demo Scenario Tests (with reliability trials, including D5a 10 trials), Demo Flow Tests (including new DF3a-DF3d)
...
3. All Gemini Integration tests marked P0 (~12 tests, 35 min) - includes new G24
4. All Demo Scenario tests D1-D5 + reliability (~10 tests, 90 min)
5. Demo Flow tests DF1, DF3a-DF3d, DF11, DF12, DF14, DF22 (~9 tests, 70 min)
6. Accessibility tests A1-A8 (~8 tests, 15 min)

**Minimum Total: ~69 critical tests, ~4.5 hours**
...
| Multi-Modal (Audio+Visual) | 10 | X | X% |
...
### Backup Audio Files to Verify (NEW in v2)

| Filename | Trigger Condition | Expected Content |
|----------|-------------------|------------------|
| connection-lost.mp3 | WebSocket disconnects | "I've lost my connection..." |
| reconnecting.mp3 | During reconnect attempt | "Just a moment, re-establishing..." |
| safety-reminder.mp3 | In degraded/offline mode | "Do mind your safety whilst..." |
| connection-restored.mp3 | Successful reconnection | "Ah, splendid! Back in action..." |
...
### v2 (January 16, 2026)
- Added 18 new tests addressing critic's 10 identified gaps
- Promoted D5a (Multi-Modal Detection) from P2 to P1 with 10 reliability trials
- Added DF3a-DF3d for explicit camera setup stage verification
- Added G22-G26 for personality arc, name frequency, token personalization, keyword mapping, and session resumption
- Added E17a-E17d and E19 for backup audio file playback verification
- Added F36 for sensitivity slider impact verification
- Updated test counts and time estimates
- Added backup audio files reference to appendix
- Total tests increased from 184 to 202

---

*End of Test Plan v2 - Sir Reginald*
